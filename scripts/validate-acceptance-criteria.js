...
function parseAcceptanceCriteria(issueBody, issueNumber) {
  ...
  while ((match = checkboxRegex.exec(acSection)) !== null) {
    ...
    acceptanceCriteria.push({
      id: `US-${String(issueNumber).padStart(3, '0')}-AC-${index}`,
      text,
      checked,
      coveredBy: [],
    });
...
function generateValidationResults(issueNumber, acceptanceCriteria) {
  ...
  const results = {
    ...
    coverage: acceptanceCriteria.length > 0 ? Math.round((completedAC.length / acceptanceCriteria.length) * 100) : 0,
    details: {
...
async function main() {
  ...
  // Parse AC
  let acceptanceCriteria = parseAcceptanceCriteria(issue.body, ISSUE_NUMBER);
